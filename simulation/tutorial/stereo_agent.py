...
def _render(sim, display, filename:str, depth=False):
    for _ in range(100):
        ...
        if display:
            cv2.imwrite(filename, stereo_pair)
...
def main(display=True):
    ...
    if display:
        ...
        # comment out for headless purpose.
    ...
    _render(sim, display, filename='rgb_stereo_img.png')
    sim.close()
    ...
    _render(sim, display, depth=True, filename='depth_stereo_img.png')
